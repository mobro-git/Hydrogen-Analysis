...
categorize <- function(table) {
  x <- table %>%
    ...
    mutate(Scenario = paste(nz,".",fcev,".",bev,".",td,".",fut,".",pem,sep = "")) %>%
...
fuel <- function(table) {
  x <- table %>%
    mutate(Fuel = case_when(
      str_detect(Processset, "ELC") ~ "Electric",
      str_detect(Processset, "GSL") ~ "Gasoline",
      str_detect(Processset, "JTF") ~ "Jet Fuel",
      str_detect(Processset, "B20X") ~ "Diesel",
      str_detect(Processset, "CNG") ~ "Compressed Natural Gas",
      str_detect(Processset, "DSL") ~ "Diesel",
      str_detect(Processset, "H2") ~ "Hydrogen",
      str_detect(Processset, "LPG") ~ "Propane",
      str_detect(Processset, "E85X") ~ "Gasoline",
      TRUE ~ "Other"
    )) 
}

trnsector <- function(table) {
  x <- table %>%
    mutate(trnsector = case_when(
      str_detect(Processset, "TA") ~ "Air",
      str_detect(Processset, "TB") ~ "Bus",
      str_detect(Processset, "TL") ~ "Light Duty",
      str_detect(Processset, "TH") ~ "Heavy Duty",
      str_detect(Processset, "TM") ~ "Medium Duty",
      str_detect(Processset, "TR") ~ "Rail",
      str_detect(Processset, "TS") ~ "Shipping",
      TRUE ~ "Other"
    )) 
}
